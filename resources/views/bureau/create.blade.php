...
<x-app-layout>
    ...
    <div class="mt-16 mx-28 bg-white rounded-lg max-w-7xl sm:px-6 lg:px-8 space-y-6">
        <div class="mt-16">
            <form method="post" class="p-3" action="{{route('bureau.store',$theid)}}">
            @csrf
            @method('post')
            <!-- titre -->
            <div class="mt-4">
                <x-input-label for="titre" :value="__('Titre')" />
                <x-text-input id="titre" class="block mt-1 w-full" type="text" name="titre" :value="old('titre')"  autofocus autocomplete="titre" />
